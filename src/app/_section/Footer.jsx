...
import img1 from "@/assets/images/footer-logo.svg";
    ...
    <>
      <footer className="bg-[#090909] text-[#8b8b8b] pt-16 pb-8 px-4">
        <div className="max-w-7xl mx-auto grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-10">
          ...
          <div>
            <a href="/">
              <Image src={img1} alt="Mysty Enterprises" className=" w-[100px] h-[100px]" />
            </a>
            ...
            <div className="flex flex-wrap gap-4 mt-4">
              {Social.map((item, idx) => (
                <Link key={idx} href={item.link} target="blank" className="hover:text-white transition duration-200">
                  {item.name}
                </Link>
              ))}
            </div>
          ...
          <div>
            ...
            <ul className="space-y-2">
              {Linkdata.map((item, idx) => (
                <li key={idx}>
                  <Link href={item.link} className="hover:text-white transition duration-200">
                    {item.name}
                  </Link>
                </li>
              ))}
          ...
          <div>
            ...
            <ul className="space-y-2">
              {["Web Development", "Content Writing", "Digital Marketing", "Business Solutions", "UI UX Designing"].map((service, idx) => (
                <li key={idx}>
